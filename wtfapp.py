...
def reply():
  ...
  user_latitude = request.values.get('Latitude')
  user_longitude = request.values.get('Longitude')
  ...
  try:
    ...
    if user_exists == 0 and media_msg == '0' and user_latitude is None and user_longitude is None:
    ...
    elif user_exists == 0 and user_latitude and user_longitude:
      insert_users = ''' INSERT INTO uploads(phone_number, latitude, longitude, file_name, file_blob)
        VALUES(?) '''
      cur = conn.cursor()
      cur.execute(insert_users, (sender_phone_number, user_latitude, user_longitude, "PIC URL HERE", "BLOB UNNECESSARY",))
      conn.commit()
...
def register():
  if request.method == 'POST':
    ...
    try:
      ...
      query_result = cur.fetchone()
      user_exists = query_result[0]
      # if phone number not in db, add to db
      if user_exists == 0: 
        insert_users = ''' INSERT INTO uploads(phone_number, latitude, longitude, file_name, file_blob)
          VALUES(?) '''
        cur = conn.cursor()
